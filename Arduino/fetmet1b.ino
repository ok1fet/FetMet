 /*////////////////////////////////////////////////////////////////////////////////////////////
  ...
  | |_  |  _|   | |_____| |\/| |  _|   | |  
  |  _| | |___  | |_____| |  | | |___  | |  
  |_|   |_____| |_|     |_|  |_|_____| |_|  Metostanice LoRa verze 1.b

*/
 
const char* station = "OK1FET-6>APRS:!5004.91N/01431.53E_";  // vypocet loc je v poznamkach
#define VREF              3.657f // kalibrace AD prevodniku
#define ELEVATION 225            // výška sondy v metrech nad mořem
const char* rstv = "RESETb";     // idetifikace ze doslo k reset sondy + verse sw pro orientaci
/////////////////////////////////////////////////////////////////////////////////////////////
...
const unsigned long sleepInterval = 20000; // 20000 ms = (20s)
...
// vypocet prumerna hodnota vetru
float avgSpeed(float *a, int n) {
  float s = 0;
...
// vypocet prumerneho smer vetru
float avgDir(float *a, int n) {
  float sx = 0, sy = 0;

  Serial.print("avgDir vstupni pole: ");
  for (int i = 0; i < n; i++) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    sx += cx;
    sy += syi;
    Serial.printf("Mereni %d: deg=%.2f rad=%.2f cos=%.4f sin=%.4f | sx=%.4f sy=%.4f\n",
                  i, a[i], r, cx, syi, sx, sy);
  }
  float ar = atan2(sy, sx);
  float d = degrees(ar);
  float finalDir = d < 0 ? d + 360 : d;

  Serial.printf("Souhrn: atan2(sy, sx)=%.4f rad = %.2f deg | Vysledny prumer smeru: %.2f°\n",
                ar, d, finalDir);
  return finalDir;
}
...
// Zkontroluj pin 13 - pokud je LOW, rozsvit LED na pinu 25
if (digitalRead(OLED_ON_PIN) == LOW) {
...
// vypocet prumernych hodnot a maximalni hodnoty vwreu
  float avgSp  = avgSpeed(rtc_windSpeedBuf, 13);
  float avgDr  = avgDir(rtc_windDirBuf, 13);
