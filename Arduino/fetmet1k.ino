...
float rain() {
  ...
  rainSum1h = 0;
  for (int i = 0; i < 12; i++)  rainSum1h += rainBuf1h[i];

  rainSum24h = 0;
  for (int i = 0; i < 288; i++) rainSum24h += rainBuf24h[i];

  DebugPrintf("1h: %d | 24h: %d\n", rainSum1h, rainSum24h);

  rainBuf1hPoradi = (rainBuf1hPoradi + 1) % 12;
  rainBuf24hPoradi = (rainBuf24hPoradi + 1) % 288;
  rainCount5min = 0;
...
//////////////////////////////////////////////////////////////////////////////////////////////  
    ...
    // Odeslání zprávy
        ...
        rainSum1h       = 0;
        rainBuf1hPoradi = 0;
        for (int i = 0; i < 12; ++i) rainBuf1h[i] = 0;
  ...
  // memset(rainBuf1h, 0, sizeof(rainBuf1h));
...
for (int i = 0; i < 12; i++) rainSumCheck += rainBuf1h[i];
