/*////////////////////////////////////////////////////////////////////////////////////////////
  ...
  | |_  |  _|   | |_____| |\/| |  _|   | |  
  |  _| | |___  | |_____| |  | | |___  | |  
  |_|   |_____| |_|     |_|  |_|_____| |_|  Metostanice LoRa verze 1.i
*/
/////////////////////////////////////////////////////////////////////////////////////////////
const char* station = "OK1FET-99>APRS:!5004.91N/01431.53E_";  // vypocet loc je v poznamkach
#define VREF              3.657f // kalibrace AD prevodniku nové VREF = 3.657 × (4.17 hw / 4.34 mp ) ≈ 3.657 × 0.960 ≈ 3.513
#define ELEVATION 225            // výška sondy v metrech nad mořem
const char* rstv = "000/000g000r000_RESETi";     // identifikace resetu sondy + verse SW
/////////////////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////////////////  
    case ESP_SLEEP_WAKEUP_EXT0: {
      ...
      rainCount5min++;
      DebugPrintf("💧 Doslo preruseni od srazkomeru v %d/18  %d/11 cyklu pocet je %d\n", cyklus1s ,cyklus20s, rainCount5min);
...
//////////////////////////////////////////////////////////////////////////////////////////////
    case ESP_SLEEP_WAKEUP_TIMER: {
      ...
      DebugPrintf("⏱️ Probuzeni %d/18 v %d/11 cyklu\n", cyklus1s, cyklus20s);
...
// každých 18s + 3s základní meteorologické měření = 20s
      if (cyklus1s % 18 == 0) {
        cyklus1s   = 0;
        cyklus20s++;
        DebugPrintf("📡 Doslo k preruseni od casovace 20s je %d/11 cyklus.📡\n", cyklus20s);
...
//konec 20 sekundoveho cyklu
        DebugPrintf("Cyklus %d/11 | %d speed pulse = %.1f m/s  smer %.0f° rain pulse %d bat %.2fV\n", cyklus20s, SpeedPulseCount, (wSpeed*0.44704), wDir, rainCount5min, readBat());

        // -----------------------------------------------------------------
        // Agregace a odeslání dat po 5 minutach = 11 cyklech
        // -----------------------------------------------------------------
        // 11 cyklu x 20 sekund + 80 tx msg = 300 sekund = 5 minut
        if (cyklus20s >= 11) {
...
// --- Pokud ještě nejsme u 11 cyklů, naplánujeme další probuzení za 20s ( + déšť ) ---
  ...
  DebugPrintf("😴 set %.d ms DeepSleep!", sleepInterval);
  delay(50);
  esp_deep_sleep_start();
